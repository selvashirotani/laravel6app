...
@if(!empty($items))
    ...
    <a class="back-btn" href="/admin/reviews/form?id={{$item->review_id}}">編集</a>
    
    <a class="back-btn" href="{{ route('admin_reviews.destroy') }}"
        onclick="event.preventDefault();
                        document.getElementById('delete-form').submit();">
        削除
    </a>
   
    <form id="delete-form" action="{{ route('admin_reviews.destroy',['$id'])}}" method="POST" class="d-none">
    <input type="hidden" value="{{$item->review_id}}" name="products_id">
            @csrf
    </form>
